...
if ($_SESSION['id']) {
	if(isset($_POST['submit'])){
		...
		if ($upload){
			$insert = mysqli_query($con, "INSERT INTO major_genre(gen_id, title, title_meaning, cover_img, author, kiny_title, eng_title, fre_title, swah_title, english, french, swah, kiny, release_date) VALUES('{$gen_id}', '{$title}', '{$title_meaning}', '{$img_name}', '{$author}', '{$kiny_title}', '{$title}', '{$fr_title}', '{$swa_title}', '{$english}', '{$french}', '{$swahili}', '{$kinyarwanda}', '{$release}')");

			if ($insert){
				header("location: ../gen_maj.php?new=$gen_id");
			}else{
				header("location: ../gen_maj_add.php?err");
			}
		}else{
			header("location: ../gen_maj_add.php?err");
		}
