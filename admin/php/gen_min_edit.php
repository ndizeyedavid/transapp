...
if ($_SESSION['id']) {
	if(isset($_POST['submit'])){
		...
		$update = mysqli_query($con, "UPDATE minor_genre SET title='{$title}',title_meaning='{$title_meaning}',cover_img='{$img_name}',author='{$author}',kiny_title='{$kiny_title}',eng_title='{$eng_title}',fre_title='{$fre_title}',swah_title='{$swa_title}',english='{$english}',french='{$french}',swah='{$swahili}',kiny='{$kinyarwanda}',release_date='{$release}' WHERE id  = '{$id}'");

		if ($update){
			header("location: ../gen_min.php?new=$gen_id");
		}else{
			header("location: ../gen_min_edit.php?err");
		}
